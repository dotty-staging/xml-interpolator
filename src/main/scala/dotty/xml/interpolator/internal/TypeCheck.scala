          ...
          attribute.value match {
            case Seq(Placeholder(id)) =>
              ...
              val expected = attribute.isNamespace match {
                case true => Seq('[String].asTypeTree.tpe)
                case _ => Seq(
                  '[String].asTypeTree.tpe,
                  '[collection.Seq[scala.xml.Node]].asTypeTree.tpe,
                  '[Option[collection.Seq[scala.xml.Node]]].asTypeTree.tpe
                )
              }
